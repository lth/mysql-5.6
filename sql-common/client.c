...
static uchar *
encode_attrs(uchar *buf, HASH *attrs, size_t attrs_length)
{
  buf= net_store_length(buf, attrs_length);

  /* check if we have attributes */
  if (attrs && my_hash_inited(attrs))
  {
    ulong idx;

    /* loop over and dump the attributes */
...
send_client_connect_attrs(MYSQL *mysql, uchar *buf)
{
  ...
  {
    if (mysql->options.extension)
      buf= encode_attrs(buf, &mysql->options.extension->connection_attributes,
                        mysql->options.extension->connection_attributes_length);
    else
      encode_attrs(buf, NULL, 0);
  }
  return buf;
}
                                            ...
                                            char **out_buff, int *out_len)
{
  ...
  *out_len = 0;
  ...
  buff= my_malloc(USERNAME_LENGTH + data_len + 1 + NAME_LEN + 2 + NAME_LEN +
                  connect_attrs_len + MAX_VARIABLE_STRING_LENGTH,
                  MYF(MY_WME | MY_ZEROFILL));
    ...
    MCPVIO_EXT *mpvio,const uchar *pkt, int pkt_len, my_bool *result)
{
  ...
  if (!ctx->change_user_buff) {
    ...
    error = prep_send_change_user_packet(mpvio, pkt, pkt_len,
...
#define MAX_ATTR_STORAGE_LENGTH 65536
                                        ...
                                        char **buff_out, int *buff_len)
{
  ...
  DBUG_ASSERT(connect_attrs_len < MAX_ATTR_STORAGE_LENGTH);
 ...
 *
 * Value of `*error` is `true` when an error occurs.
 *
 * Returns net_async_status
 */
static int run_ssl_connect(MCPVIO_EXT *mpvio, my_bool *error) {
  ...
  if (!mysql->connector_fd) {
    ...
    // net->ssl as well. This assignment makes sure that SSL this
    ...
    MCPVIO_EXT *mpvio,const uchar *pkt, int pkt_len, my_bool *result)
{
  ...
  if (!ctx->change_user_buff) {
    error = prep_client_reply_packet(mpvio, pkt, pkt_len,
                                     &ctx->change_user_buff,
                                     &ctx->change_user_buff_len);
    if (error) {
      goto end;
  ...
  if (ctx->ssl_exchange_state == SSL_REQUEST) {
    net_async_status status =
      my_net_write_nonblocking(
        net, (uchar*)ctx->change_user_buff, SSLREQUEST_LENGTH, &error);
    if (status == NET_ASYNC_NOT_READY) {
      DBUG_RETURN(NET_ASYNC_NOT_READY);
    }
    if (error) {
...
/**
 * Logic is the same as `client_mpvio_write_packet` but for nonblocking
 * context.
 *
 * @param result[out]   `-1` when error happens.
 *
 * @return Returns `int` that stands for `net_async_status`
 */
...
mysql_send_query(MYSQL *mysql, const char *query, ulong length)
{
  ...
  if (query_attrs_len > 0) {
    my_bool ret;
    uchar* buf= my_malloc(query_attrs_len + MAX_VARIABLE_STRING_LENGTH,
                          MYF(MY_WME | MY_ZEROFILL));

    uchar* end= send_client_query_attrs(mysql, buf);
    query_attrs_len = end - buf;

    ret= (*mysql->methods->advanced_command)(mysql, COM_QUERY_ATTRS, buf,
...
mysql_send_query_nonblocking(MYSQL *mysql, const char *query, int *error)
{
  ...
  if (query_attrs_len > 0) {
    ...
    ret= (*mysql->methods->advanced_command_nonblocking)(mysql,
                                                         COM_QUERY_ATTRS, buf,
                                                         query_attrs_len,
                                                         (uchar*) query,
                                                         mysql->async_query_length,
                                                         1, NULL, &error_bool);
    my_free(buf);
...
static int
add_attributes(MYSQL *mysql, const void *arg1, const void *arg2,
               HASH* attrs, size_t* attrs_length)
{
  DBUG_ENTER("add_attributes");
  LEX_STRING *elt;
  char *key, *value;
  ...
  /*
    ...
    */
  if (attr_storage_length + *attrs_length > MAX_ATTR_STORAGE_LENGTH)
  {
    set_mysql_error(mysql, CR_INVALID_PARAMETER_NO, unknown_sqlstate);
    DBUG_RETURN(1);
  }

  if (!my_hash_inited(attrs))
  {
    if (my_hash_init(attrs, &my_charset_bin, 0, 0, 0,
                     (my_hash_get_key) get_attr_key, my_free, HASH_UNIQUE))
    {
      set_mysql_error(mysql, CR_OUT_OF_MEMORY, unknown_sqlstate);
      DBUG_RETURN(1);
    }
  ...
  value[value_len]= 0;
  if (my_hash_insert(attrs, (uchar *) elt))
  {
    /* can't insert the value */
    my_free(elt);
    set_mysql_error(mysql, CR_DUPLICATE_ATTR, unknown_sqlstate);
    DBUG_RETURN(1);
  }
               ...
               const void *arg1, const void *arg2)
{
  ...
  case MYSQL_OPT_CONNECT_ATTR_ADD:
    {
      ENSURE_EXTENSIONS_PRESENT(&mysql->options);
      DBUG_RETURN(add_attributes(mysql, arg1, arg2,
                                 &mysql->options.extension->
