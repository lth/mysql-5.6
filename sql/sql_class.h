...
private:
  ...
  ha_rows offset;
  /// Number of rows limit + offset, @see
  //select_union_direct::send_data()
  ha_rows limit;
...
public:
  ...
  {
    // Only called for top-level
    // select_results, usually select_send
  ...
  {
    // Only called for
    // sp_cursor::Select_fetch_into_spvars
    DBUG_ASSERT(false); /* purecov: inspected */
  }
};
